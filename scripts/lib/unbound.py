...
# Copied here to keep the plugin self-contained (no git submodule / no pip dependency).
...
# NOTE: process_stop_event() and main() from the upstream websentry-ai/setup
# copy have been removed. The plugin uses scripts/hook-handler.py as its entry
# point and imports individual functions from this module. The legacy standalone
# handler logic lives only in the upstream repo.
